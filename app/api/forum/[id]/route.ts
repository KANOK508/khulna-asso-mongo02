...
export async function GET(
  ...
  try {
    await connectDB();
    const { id } = await params;
    const post = (await ForumPost.findById(id).lean()) as Record<string, unknown> | null;
    if (!post) return NextResponse.json({ error: "Post not found" }, { status: 404 });
    ...
    const authorIds = [post.authorId as string, ...comments.map((c) => c.authorId as string)];
    const uniqueIds = [...new Set(authorIds)];
    const userCol = await getUserCollection();
    const users = await userCol
      .find(
    ...
    return NextResponse.json({
      ...
      author: userMap.get(post.authorId as string) ?? { name: "Unknown" },
      comments: comments.map((c) => ({
        ...c,
        id: (c._id as { toString(): string }).toString(),
        author: userMap.get(c.authorId as string) ?? { name: "Unknown" },
      })),
    });
  } catch (err) {
    console.error(err);
